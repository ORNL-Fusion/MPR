function y = emitted_part_surf_intersec(x0,y0,z0,phi,th,phi_loc,th_loc,phi_s,th_s,A,SX1,RX1,BX1,CX1,SX2,RX2,BX2,CX2,SY1,RY1,BY1,CY1,SY2,RY2,BY2,CY2,bx,by,initz0, dx, dy,event)
    ...
    if (sin(phi) >= 0.0 && cos(phi) >= 0.0 )
        ...
        if (sin(phi)==0) %tan(phi)=0 -> ys=y0
            ...
            for i=1:n
                ...
                r = (x0+dx)+i*dx;
                try
                    [xs,fval,exitflag,outinfo]=fzero(@(x) zpx(x,x0,z0,th,phi)-zs(x,y0,A,SX1,RX1,BX1,CX1,SX2,RX2,BX2,CX2,SY1,RY1,BY1,CY1,SY2,RY2,BY2,CY2,bx,by),[l r]);
                catch
        ...
        elseif (cos(phi)==0)
            ...
            for i=1:n
                ...
                r = (y0+dy)+i*dy;
                try
                    [ys,fval,exitflag,outinfo]=fzero(@(y) zpy(y,y0,z0,th,phi)-zs(x0,y,A,SX1,RX1,BX1,CX1,SX2,RX2,BX2,CX2,SY1,RY1,BY1,CY1,SY2,RY2,BY2,CY2,bx,by), [l r]);
                catch
                    %disp('no solution found by fzero (err); set redep=6 & exitflag=-6, xs=x0, ys=y0');
            ...
            for i=1:n
                ...
                r = (y0+dy)+i*dy;
                try
                    [ys,fval,exitflag,outinfo]=fzero(@(y) zpy(y,y0,z0,th,phi)-zs(x0+(y-y0)/tan(phi),y,A,SX1,RX1,BX1,CX1,SX2,RX2,BX2,CX2,SY1,RY1,BY1,CY1,SY2,RY2,BY2,CY2,bx,by), [l r]);
                catch
                    %disp('no solution found by fzero (err); set redep=6 & exitflag=-6, xs=x0, ys=y0');
        ...
        q=zs(xs,ys,A,SX1,RX1,BX1,CX1,SX2,RX2,BX2,CX2,SY1,RY1,BY1,CY1,SY2,RY2,BY2,CY2,bx,by);
