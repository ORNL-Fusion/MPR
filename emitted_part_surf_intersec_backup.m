function y = emitted_part_surf_intersec_backup(x0,y0,z0,phi,th,Ax,fx,Ay,fy)
...
%%and use it to pass only one variable to zs function
 ...
 elseif (th<0 && th<=pi/2.0)  %particle emitted upwards
     

     
        %%%%%%%%%%%%%%%%%%HOW TO KNOW IF HIT SURFACE OR NOT%%%%%%%%%%% 
        
        
       
    if (Ax==0) %back to 1D sinusoidal in y

            ys=fzero(@(y) zpy(y,y0,z0,th,phi)-zs(x0+(y-y0)/tan(phi),y,Ax,fx,Ay,fy), [y0, y0+2*pi*sign(sin(phi))]); 
            xs=x0+(ys-y0)*tan(phi);
            q=zs(xs,ys,Ax,fx,Ay,fy);
     ...
     elseif (Ay==0)
            ...
            xs=fzero(@(x) zpx(x,x0,z0,th,phi)-zs(x,y0+(x-x0)*tan(phi),Ax,fx,Ay,fy), [x0, x0+2*pi*sign(cos(phi))]); 
            ys=y0+(xs-x0)*tan(phi); 
            q=zs(xs,ys,Ax,fx,Ay,fy); 
     ...
     S7=['invalid value for emitted particles theta, ', num2str(th)];
     S8=['assuming particle was not re-deposited'];
     disp(S7)
     disp(S8)
...
%%NO NEED TO CALCULATE THE ANGLE AT THE INTERSECTING POINT 
...
if(locang<0.0)
    locang=-acos(cosa);
    S9=[' For particle ',num2str(p),' impacting in xs = ',num2str(xs), ' ys =',num2str(ys),' ys =',num2str(q)];
    S10=[' angle =', num2str(180/pi*locang), '< 0; -angle will be taken '];    
    disp(S9);
    disp(S10);
end
if (cosa<0.0)
